...
function unzip(zipPath, destinationPath, tempFolder){
...
function decryptZip(encryptedFolderPath, tempFolder, password, callback) {
	...
	ws.on("finish", function (err) {
		if(err){
			...
			ws.close();
			var newPath = path.join(path.normalize(encryptedFolderPath+"/.."), encode(crypto.randomBytes(32)));
			fs.renameSync(encryptedFolderPath, newPath);
			fs.unlinkSync(newPath);
			unzip(tempArchivePath, encryptedFolderPath, tempFolder);
		}
	});
