...
function encryptFile(inputPath,  destinationPath, password){
	...
	console.log("zipPath:", inputPath);
	console.log("outpath:", destinationPath);
	...
	cipher.setAAD(aad);
	let cipherStream = rs.pipe(cipher);


	console.log(cipherStream);
	...
	cipherStream.on('end', function () {
		...
		ws.write(dataToAppend, function (err) {
			ws.close();
			deleteFolder(path.dirname(inputPath));
