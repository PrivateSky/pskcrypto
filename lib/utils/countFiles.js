...
function countFiles(inputPath, callback) {
    ...
    fs.stat(inputPath, (err, stats) => {
        if (err) {
            return callback(createOpenDSUErrorWrapper(`Failed to get stats for file <${inputPath}>`, err));
        }
        ...
        fs.readdir(inputPath, (err, files) => {
            if (err) {
                return callback(createOpenDSUErrorWrapper(`Failed to read dir <${inputPath}>`, err));
            }
                ...
                fs.stat(filePath, (err, stats) => {
                    ...
                    if (stats.isDirectory()) {
                        ...
                        countFiles(folderPath, (err, filesNumber) => {
                            if (err) {
                                return callback(createOpenDSUErrorWrapper(`Failed to get count files in folder <${folderPath}>`, err));
                            }
...
function countZipEntries(inputPath, callback) {
    ...
    yauzl.open(inputPath, {lazyEntries: true}, (err, zipFile) => {
        if (err) {
            return callback(createOpenDSUErrorWrapper(`Failed to open zip file <${inputPath}>`, err));
        }
