function PskCrypto() {
    ...
    const keyGenerator = require("./ECKeyGenerator").createECKeyGenerator();
    ...
    this.generateKeyPair = function(options, callback) {
        if (typeof options === "function") {
            callback = options;
            options = undefined;
        }

        const keys = keyGenerator.generateKeyPair(options);
        return callback(undefined, keys.publicKey, keys.privateKey);
    };

    this.convertKeys = keyGenerator.convertKeys;

    this.sign = (data, privateKey) => {
    ...
    this.verify = (data, publicKey, signature) => {
        if (typeof data === "string") {
            data = Buffer.from(data);
        }
