...
function PskCrypto() {
    ...
    this.encryptObject = function (inputObj, passphrase, depth = 0, callback) {
        const archiver = new PskArchiver();

        archiver.zipInMemory(inputObj, depth, (err, zippedObj) => {
            if (err) {
                return callback(err);
            }

            const cipherText = utils.encrypt(zippedObj, passphrase);
            callback(null, cipherText);
        });
    };

    this.decryptObject = function (encryptedData, passphrase) {

        const obj = utils.decryptBuffer(encryptedData, passphrase);
        return JSON.parse(obj.toString());
    };

    this.encrypt = function (data, passphrase) {
        if (Buffer.isBuffer(data)) {
            return utils.encryptBuffer(data, passphrase);
        }
        ...
        return utils.encryptBuffer(Buffer.from(data), passphrase);
    };

    this.decrypt = function (encryptedData, passphrase) {
        if(Buffer.isBuffer(encryptedData)) {
            return utils.decryptBuffer(encryptedData, passphrase);
        }

        return utils.decryptBuffer(Buffer.from(encryptedData), passphrase);
    };
