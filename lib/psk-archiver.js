...
const fs = require("fs");
const WritableStream = require("./utils/WritableStream");
...
function PskArchiver() {
	...
	function zipObjectRecursively(obj, root,  callback){
		...
		keys.forEach( (key) => {
			...
			if(typeof obj[key] === "string"){
				...
				zipfile.addReadStream(obj[key], entryName+"/string");
			}else {
				zipObjectRecursively(obj[key], entryName, function () {
				});
			}
		});
		callback();
	...
	this.unzipInMemory = function (input, callback) {
		yauzl.fromBuffer(input, {lazyEntries: true}, function (err, zipFile) {
			...
			zipFile.on("entry", function (entry) {
				...
				zipFile.openReadStream(entry, function (err, readStream) {
					...
					readStream.pipe(ws).on("finish", function (err) {
						var splitEntry = entry.fileName.split("/");
						var type = splitEntry.pop();
						addNestedProp(obj, splitEntry, type, ws.getData());
					});
