	...
	function zipObjectRecursively(obj, root,  callback){
		...
		keys.forEach( (key) => {
			...
			if(typeof obj[key] === "string"){
				zipfile.addBuffer(new Buffer(obj[key]), entryName);
			}else if(Buffer.isBuffer(obj[key])){
				zipfile.addBuffer(obj[key], entryName);
			}else if(isStream.isReadable(obj[key])){
				zipfile.addReadStream(obj[key], entryName);
			}else {
		...
		yauzl.fromBuffer(input, {lazyEntries: true}, function (err, zipFile) {
			...
			zipFile.on("entry", function (entry) {
				zipFile.openReadStream(entry, function (err, readStream) {
					...
					let fileName = path.join(output, entry.fileName);
					let folder = path.dirname(fileName);
					$$.ensureFolderExists(folder, (err) => {
						if (err) {
							return callback(err);
