...
function PskArchiver() {
    ...
    this.zipStream = function (inputPath, output, callback) {
        ...
        countFiles.countFiles(inputPath, (err, totalFiles) => {
            if (err) {
                return callback(err);
            }
            ...
            __addToArchiveRecursively(zipFile, inputPath, "", (err) => {
                ...
                if (isStream.isWritable(output)) {
                    ...
                    $$.ensureFolderExists(output, () => {
                        ...
                        zipFile.outputStream.pipe(fs.createWriteStream(destinationPath)).on("close", () => {
                            callback(null);
                        });
                    });
                }
            ...
            function __addToArchiveRecursively(zipFile, inputPath, root = '', callback) {
                ...
                fs.stat(inputPath, (err, stats) => {
                    ...
                    if (stats.isFile()) {
                        ...
                        setInterval(() => {
                            emitProgress(totalFiles, processed);
                        }, 30);
                        callback(null);
                    } else {
    ...
    this.unzipStream = function (input, outputPath, callback) {
        ...
        countFiles.countZipEntries(input, (err, total) => {
            if(err){
                return callback(err);
            }
    ...
    function emitProgress(totalFiles, processed) {
        if (processed > totalFiles) {
            processed = totalFiles;
        }

        const progress = (100 * processed) / totalFiles;
...
utils.inherits(PskArchiver, EventEmitter);
